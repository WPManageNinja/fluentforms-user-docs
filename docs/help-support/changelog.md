...
```markdown [🐞 Bug fixes]
• Fixed a stored XSS vulnerability in cookie smartcodes
...
• Protected stored payment and integration credentials from being exposed
```

```markdown [🐞 Bug fixes]
• Fixed the Reports country map not highlighting countries on translated sites
• Fixed coupon codes being rejected for everyone on sites behind a CDN or proxy
