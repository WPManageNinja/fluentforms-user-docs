...
This option is perfect for when you need the form to act (like sending an [email notification](/setup-adminuser-email-notifications) or a [CRM integration](/integrations-overview)) but don't need to store the entry data in WordPress.

In the **Compliance Settings**, check the box next to **Delete entry data after form submission**.

> [!Warning]
> When enabled, entries are deleted immediately after submission. Fluent Forms will not store entry data, analytics, or [visual reports](/visual-representation-of-form-entries) for this form.
